...
int statespace_c2d(Statespace* ss, float Ts, char* method, float* work)
{
	...
	case 't': // Tustin
		...
		matrixf_transpose(&Bd);
		for (i = 0; i < Nxx; i++) A[i] = Ad1.data[i] + Ad2.data[i];
		for (i = 0; i < Nxu; i++) B[i] = Bd.data[i];
	...
	case 'z': // Zero-order hold
		...
		for (i = 0; i < L.size[0]; i++) {
			for (j = 0; j < L.size[1]; j++) {
				if (j < Nx) at(&L, i, j) = (i < Nx) ? A[i * Nx + j] * Ts : 0;
				else at(&L, i, j) = (i < Nx) ? B[i * Nu + j - Nx] * Ts : 0;
			}
		}
		...
		for (i = 0; i < Nx; i++) {
			for (j = 0; j < L.size[1]; j++) {
				if (j < Nx) A[i * Nx + j] = at(&L, i, j);
				else B[i * Nu + j - Nx] = at(&L, i, j);
			}
		}
	...
	case 'f': // First-order hold
		...
		for (i = 0; i < L.size[0] * L.size[1]; i++) work[i] = 0;
		for (i = 0; i < Nu; i++) at(&L, Nx + i, Nx + Nu + i) = 1;
		for (i = 0; i < Nx; i++) {
			for (j = 0; j < Nx + Nu; j++) {
				if (j < Nx) at(&L, i, j) = A[i * Nx + j] * Ts;
				else at(&L, i, j) = B[i * Nu + j - Nx] * Ts;
			}
		}
		...
		for (i = 0; i < Nx; i++) {
			for (j = 0; j < L.size[1]; j++) {
				if (j < Nx) A[i * Nx + j] = at(&L, i, j);
				else if (j < Nx + Nu) B[i * Nu + j - Nx] = at(&L, i, j);
				else work[i * Nu + j - Nx - Nu + p] = at(&L, i, j);
			}
		}
